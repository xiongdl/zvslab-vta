...
def _is_c_host(f):
    """Return whether the active lowering target uses the C host backend."""
    target = f.attrs.get("target") if f.attrs is not None else None
    if target is None:
        target = tvm.target.Target.current(allow_none=True)
    if target is None:
        return False
    host = target.host if target.kind.name == "vta" else target
    return host is not None and host.kind.name == "c"
...
def CPUAccessRewrite():
    ...
    def _ftransform(f, mod, ctx):
        ...
        def cpu_address(expr):
            if not c_host or not isinstance(expr, tvm.tir.Var):
                return expr
            if not isinstance(expr.type_annotation, tvm.ir.PointerType):
                return expr
            # C host codegen preserves refined pointer annotations when it
            # infers call_extern declarations.  Reinterpret through an
            # integer before converting to handle so both the expression and
            # declaration use VTA's opaque-handle ABI.  LLVM keeps the
            # original pointer expression unchanged.
            return tvm.tir.Cast("handle", tvm.tir.reinterpret("uint64", expr))
...
def NormalizeVTAAddressArgs():
    """Normalize VTA runtime address arguments to the opaque handle ABI.

    The VTA runtime receives device buffers as opaque handles.  C host
    codegen also sees those operands as handles, so keep the call sites and
    their declarations consistent in every host module (including the graph
    module generated by TVM itself).
    ...
    def _pointer_value(expr):
        """Return the refined pointer expression behind a handle cast."""
        if isinstance(expr, tvm.tir.Cast) and expr.dtype == "handle":
            return expr.value
        return expr

    def _as_opaque_handle(expr):
        expr = _pointer_value(expr)
        if not isinstance(expr, tvm.tir.Var) or not isinstance(
            expr.type_annotation, tvm.ir.PointerType
        ):
            return None
        # A direct Cast(handle, pointer) retains the pointer's refined type in
        # C codegen.  Reinterpreting through an integer forces the generated
        # call expression and its declaration to use the opaque handle ABI.
        return tvm.tir.Cast("handle", tvm.tir.reinterpret("uint64", expr))
    ...
    def _ftransform(f, mod, ctx):
        ...
        def _post_order(op):
            if not isinstance(op, tvm.tir.Call) or not op.op.same_as(call_extern_op):
                return op
            if not op.args or not isinstance(op.args[0], tvm.tir.StringImm):
                return op
            positions = address_args.get(op.args[0].value)
            if positions is None:
                return op
            args = list(op.args)
            for position in positions:
                if position < len(args):
                    handle = _as_opaque_handle(args[position])
                    if handle is not None:
                        args[position] = handle
            return tvm.tir.Call(op.dtype, op.op, args, span=op.span)

        body = tvm.tir.stmt_functor.ir_transform(f.body, None, _post_order, ["tir.Call"])
        return f.with_body(body)

    return tvm.tir.transform.prim_func_pass(
        _ftransform, opt_level=0, name="tir.vta.NormalizeVTAAddressArgs"
    )
