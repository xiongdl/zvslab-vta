...
The FlatBuffer has one `float32` NHWC input named `serving_default_input_5:0`
of shape `[1, 32, 32, 3]`, one `float32` output named
`StatefulPartitionedCall:0` of shape `[1, 10]`, and nine convolutions whose
output-channel progression is `40/40/40/80/80/80/160/160/160`. The model is
distributed by MLPerf Tiny under Apache-2.0; the applicable text is copied
verbatim to `../LICENSE.mlperf-tiny`.
