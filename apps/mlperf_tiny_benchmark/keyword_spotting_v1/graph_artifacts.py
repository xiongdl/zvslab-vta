"""Authenticated, atomic Graph Executor artifact export and reload for KWS."""
...
_HOST_CODEGENS = {"llvm", "c"}
_SIMULATORS = {"host", "fsim", "tsim"}


@dataclass(frozen=True)
class GraphArtifactBundle:
    """Files, immutable metadata, and the validated loaded library."""
...
def _resolve_artifact_dir(output_root, relative_artifact_dir):
    ...
    for part in relative.parts:
        ...
        if current.is_symlink():
            raise ValueError("artifact directory resolves through a symlink outside output root")
...
def _validate_metadata(name, role, model_sha256, host_codegen, simulator):
    ...
    if host_codegen not in _HOST_CODEGENS:
...
def _source_request(module_type, host_codegen):
    ...
    if lowered.startswith("llvm"):
        return "ll"
    if lowered in {"c", "cc", "cpp", "c_source"}:
        return "c"
...
def _require_host_source(sources, host_codegen):
    ...
    if not any(
        item["available"] and item["bytes"] and item["source_format"] == expected
        for item in sources
    ):
...
def _read_sources(bundle_dir, manifest):
    ...
    for index, entry in enumerate(entries):
        ...
        if not entry["available"]:
            ...
            records.append({"available": False, "bytes": None, "source_format": entry.get("source_format")})
            continue
...
def _bundle(bundle_dir, manifest, module, validated):
...
def _manifest_for(stage, factory, artifact_name, artifact_role, model_sha256,
    ...
    for index, source in enumerate(sources):
        if source["available"]:
            path = source_dir / f"{index:02d}-module.{source['source_format']}"
    ...
    return manifest, tuple(source_paths)
...
def export_graph_bundle(factory, output_root, relative_artifact_dir, *, artifact_name,
    ...
    """Export, validate, atomically publish, and reload one graph bundle."""
...
def load_graph_bundle(output_root, relative_artifact_dir):
    """Reload and validate every authenticated file and symbol contract."""
    ...
    validated = (*_read_files(bundle_dir, manifest), _read_sources(bundle_dir, manifest))
    ...
    if set(expected) & set(forbidden):
        raise RuntimeError("artifact symbol contract has overlapping expected and forbidden symbols")
    module = tvm.runtime.load_module(str(validated[0]["library"]))
    _validate_symbols(module, expected, forbidden)
    return _bundle(bundle_dir, manifest, module, validated)
